...
from sqlalchemy.orm import Query, Session, joinedload, registry, relationship
...
def test_case():
    ...
    query = query.options(joinedload(Payment.packages).joinedload(Package.baskets))
    result = query.all()

    assert len(result) == 1
    assert len(messages) == 1
    assert re.search(
        r"FROM payments LEFT OUTER JOIN packages AS packages_1 ON payments.id = packages_1.payment_id LEFT OUTER JOIN baskets AS baskets_1 ON packages_1.id = baskets_1.package_id WHERE",
        messages[0],
    )
