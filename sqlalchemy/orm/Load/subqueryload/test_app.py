...
from sqlalchemy.orm import Query, Session, registry, relationship, subqueryload
...
class Basket(Base):
    ...
    package = relationship(
        "Package", foreign_keys=[package_id], back_populates="baskets"
    )
    product = relationship(
        "Product", foreign_keys=[product_id], back_populates="baskets"
    )
...
class Package(Base):
    ...
    payment = relationship(
        "Payment", foreign_keys=[payment_id], back_populates="packages"
    )
...
def test_case():
    ...
    query = query.options(subqueryload(Payment.packages).subqueryload(Package.baskets))
    result = query.all()

    assert len(result) == 1
    assert re.search(r"FROM \(SELECT payments.id AS payments_id FROM payments WHERE payments.method = \?\) AS anon_1 JOIN packages ON anon_1.payments_id = packages.payment_id", messages[1])
    assert re.search(r"FROM \(SELECT payments.id AS payments_id FROM payments WHERE payments.method = \?\) AS anon_1 JOIN packages AS packages_1 ON anon_1.payments_id = packages_1.payment_id JOIN baskets ON packages_1.id = baskets.package_id", messages[2])
